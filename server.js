...
function dailyCloseWorkbook(data, fromDate, toDate = fromDate, reportTitle = 'إقفال اليومية') {
  ...
  const rows = [
    ...
    ['الملخص السريع'],
    ['إجمالي الوارد', incoming], ['إجمالي المصروف', outgoing], ['صافي دخل اليوم', incoming - outgoing], ['الرصيد الحالي للمركز', currentBalance], [],
  ];
  ...
  addSection('قطع الغيار المستخدمة في الزيارات', ['العميل', 'كود العميل', 'كود الزيارة', 'القطع والكميات', 'قيمة القطع'], visitsWithParts.map((visit) => {
    const customer = data.customers.find((item) => item.code === visit.customerCode) || {};
    return [customer.name, visit.customerCode, visit.code, visit.partsCodes, visit.partsTotal];
  }));
...
function reportHtml(data, fromDate, toDate, title) {
  ...
  const rows = accounts.map((a) => `<tr><td>${escape(a.executionDate || a.date)}</td><td>${escape(a.description)}</td><td>${escape(a.type)}</td><td>${escape(a.code)}</td><td>${escape(a.notes)}</td><td>${a.direction === 'صادر' ? a.paid : 0}</td><td>${a.direction === 'وارد' ? a.paid : 0}</td><td>${a.balance}</td></tr>`).join('');
