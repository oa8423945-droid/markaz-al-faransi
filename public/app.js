...
function openCustomerRecord(code) {
  ...
  $('#clientRecordContent').innerHTML = `<div class="panel customer-record-head"><div class="avatar">${esc(customer.name.slice(0, 1))}</div><div class="customer-record-identity"><h2>${esc(customer.name)}</h2><div class="customer-meta"><span>الكود: <b>${esc(customer.code)}</b></span><span>التليفون: <b>${esc(customer.phone)}</b></span><span>العربية: <b>${esc(customer.carType)}</b></span><span>اللوحة: <b>${esc(customer.plate)}</b></span></div></div><div class="record-stat"><small>إجمالي الزيارات</small><strong>${fmt(visits.length)}</strong></div><div class="record-stat"><small>إجمالي الدخل</small><strong>${fmt(income)} ج</strong></div><div class="record-stat debt-receivable"><small>المستحق على العميل</small><strong>${fmt(customer.dueFromCustomer)} ج</strong></div><div class="record-stat debt-payable"><small>المستحق على المركز</small><strong>${fmt(customer.dueFromCenter)} ج</strong></div><button id="recordNewVisit" class="primary">＋ زيارة جديدة</button></div><div class="panel visit-summary"><div class="panel-head"><div><h2>سجل الزيارات</h2><p>اضغط على أي زيارة لعرض تفاصيلها.</p></div></div>${visits.length ? table([...visits].reverse()) : '<div class="empty-state">لا توجد زيارات لهذا العميل.</div>'}</div>`;
...
function renderAccounts() {
  ...
  const rows = [...state.accounts].reverse().map((account) => `<tr class="account-row" data-account-code="${esc(account.code)}" tabindex="0"><td><b>${esc(account.code)}</b></td><td>${esc(account.executionDate || account.date)}</td><td><span class="account-direction ${account.direction === 'وارد' ? 'in' : 'out'}">${esc(account.direction || '—')}</span></td><td>${esc(account.type)}</td><td>${esc(account.customerCode || '—')}<br><small>${esc(account.customerName || '')}</small></td><td>${esc(account.supplierCode || '—')}<br><small>${esc(account.supplierName || '')}</small></td><td>${fmt(account.paid)} ج</td><td>${esc(account.paymentMethod || '—')}</td><td><b>${fmt(account.balance)} ج</b></td></tr>`).join('');
  $('#accountsTable').innerHTML = `<div class="table-wrap"><table><thead><tr><th>كود الحركة</th><th>تاريخ التنفيذ</th><th>وارد/صادر</th><th>النوع</th><th>العميل</th><th>المورد</th><th>المبلغ الفعلي</th><th>طريقة الدفع</th><th>رصيد المركز</th></tr></thead><tbody>${rows}</tbody></table></div>`;
